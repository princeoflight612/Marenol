...
void main()
{
    float radius = sqrt((v_vTexcoord.x-0.5)*(v_vTexcoord.x-0.5)+(v_vTexcoord.y-0.5)*(v_vTexcoord.y-0.5));
    float u_offset=0.0;
    //vec3 maskIntensity = vec3(u_intensity*texture2D( u_texture, v_vTexcoord ).w+u_offset+v_vTexcoord.y*0.25,0.,0.);
    float u_value = intensity*(0.5 + 0.5 * sin(40.0*(sqrt(radius)-0.5*time)));
