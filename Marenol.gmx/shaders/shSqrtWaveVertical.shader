...
void main()
{
    ...
    float u_value = sign(v_vTexcoord.y-0.5)*intensity*(0.5 + 0.5 * sin(40.0*(sqrt(abs(v_vTexcoord.y-0.5))+0.5*time)));
    ...
    gl_FragColor = v_vColour * vec4(shiftedColour,texColour.w*intensity*abs(u_value)*abs(v_vTexcoord.y-0.5));
}
